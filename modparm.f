...
!! SWAT-WE, Junyu Qi, 07/2023\\\

      !!added for pysical-based soil temperature simulation mudule
      !! for (nly)
      real, dimension (:), allocatable :: a_sol,b_sol,c_sol,d_sol
      real, dimension (:), allocatable :: p_sol,q_sol,k_int
      real, dimension (:), allocatable :: ks_coe
      !! for (ihru)
      real, dimension (:), allocatable :: a_sno,b_sno,c_sno,d_sno
      real, dimension (:), allocatable :: p_sno,q_sno,k_ss,ca_sno
      real, dimension (:), allocatable :: k_sno,d_tmps,snotmp1,sno_hru1
      real, dimension (:), allocatable :: sur_tmp,bot_tmp,t_bare,t_snow
      real, dimension (:), allocatable :: sno_den,sno_dep
      real, dimension (:), allocatable :: c_coe,eff_coe
      !! for (nly,ihru)
      real, dimension (:,:), allocatable :: ca_sol,k_sol,sol_ice
      real, dimension (:,:), allocatable :: sol_wc,sol_wcv,sol_orgv
      real, dimension (:,:), allocatable :: sol_minv
      real, dimension (:,:), allocatable :: sol_thic,sol_org,sol_icev
      real, dimension (:,:), allocatable :: sol_tmp1,sol_pormm
      real, dimension (:,:), allocatable :: d_ice, sol_air,d_sols
      real, dimension (:,:), allocatable :: sol_satu, sol_kd,sol_ke
      real, dimension (:,:), allocatable :: sol_ksat
      real, dimension (:,:), allocatable :: sol_zc
      real, dimension (:,:), allocatable :: k_coe
      integer :: nly_sub,nly1
      real, dimension (:), allocatable :: ddepth
      real, dimension (:), allocatable :: snosurtmp,ambtmp
      real, dimension (:), allocatable :: sno_por,snoco 
      real, dimension (:), allocatable :: presnotmp
	real:: eff_conr,eff_conr1   
      real, dimension (:), allocatable :: sno_iday	
	real, dimension (:,:), allocatable :: sol_nd
	real, dimension (:), allocatable ::soltmp_50,soltmp_100,soltmp_150
	real, dimension (:), allocatable ::soltmp_200,soltmp_300,soltmp_500
	real, dimension (:), allocatable ::soltmp_1000
	real, dimension (:), allocatable ::sub_sur_tmp,sub_soltmp_50
      real, dimension (:), allocatable ::sub_soltmp_100,sub_soltmp_150
      real, dimension (:), allocatable ::sub_soltmp_200,sub_soltmp_300
      real, dimension (:), allocatable ::sub_soltmp_500,sub_soltmp_1000
	 ...
	 !!============= Soil temperature Control ===============
       ...
       ! iwtmp_sub ::    0=default ; 1= Ficklin's  subbasin water temperaute; 2= Qi's hru/subbasin water temperature WHEN tswat=1
       ! isurface_tmp ::   0= soil surface temperature; 1= quilibrium temperature
       !NOTE: need to adjust paramaters in reasbsn.f for Ficklin's method.
        
	 integer:: tswat=1
	 integer:: iwtmp_sub=2
	 integer:: isurface_tmp=0
	 integer:: iwtmp_rch=1
       integer:: wtmp_option =2  
      ...
      real,dimension(:),allocatable:: wht_surq
      real,dimension(:),allocatable:: wht_sepbtm
      real,dimension(:),allocatable:: wht_latq
      real,dimension(:,:),allocatable:: wht_flat  
      real,dimension(:),allocatable:: sub_gwq_deep
      real,dimension(:), allocatable:: rch_hwattemp_pre

      real,dimension(:,:), allocatable:: rch_hwattemp
