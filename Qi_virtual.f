...
!! write daily HRU output
      ...
      !!>>>>>>>>>>>>>>>>>>>>>>Junyu Qi TSWAT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      !if ((iprint==1.or.iprint==3) .and. curyr > nyskip) call hruday1
      ! if ((iprint==1.or.iprint==3) .and. curyr > nyskip) call hrudayt
      !!<<<<<<<<<<<<<<<<<<<<<<Junyu Qi TSWAT>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   
...
!! sum HRU results for subbasin 
 ...
 !       sub_tileq(sb) = sub_tileq(sb) + tileq(j) * hru_fr(j)      !! jane f
      ...
      sub_wtmp_gwdp(sb)=sub_wtmp_gwdp(sb)+wtmp_gwq(j)*gw_qdeep(j)
     &                    *hru_fr(j)
      end if
...
!        sub_bactlp(sb) = sub_bactlp(sb) / sub_fr(sb)
      ...
      !! SWAT-WE, Junyu Qi, 07/2023\\\
       ...
       if ( iwtmp_sub == 0) then  !! Junyu Qi  
        !if (sub_wyld(sb) > 0.1) then
        if (sub_wyld(sb) > 0.000001) then    !! Junyu Qi
          sub_wtmp(sb) = sub_wtmp(sb) / sub_wyld(sb)
        else
          sub_wtmp(sb) = 0.0
        end if
