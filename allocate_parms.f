...
!  added 1/27/09 when making septic changes
      ...
      allocate (sol_tmp(mlyr+10,mhru))   !!  Junyu Qi///////////////////////////////////////
...
!!    LID general
	...
	allocate(rspc_a(mhru))	
      ...
      !!>>>>>>>>>>>>>>>>>>Soil and Snow Temperature Module >>> Junyu Qi>>>>>>>>>>>>>>>>
      ...
      allocate (t_snow(mhru))
      allocate (d_tmps(mhru))
      ...
      !!<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<Junyu Qi<<<<<<<<<<<<<<<<<<<<<<<<< 
       ...
       allocate (wht_surq(mhru))
       allocate (wht_sepbtm(mhru))
       allocate (wht_latq(mhru))
       allocate (wht_flat(mlyr,mhru))
       allocate (sub_gwq_deep(msub))
 
	  allocate(wtmppara(msub,3,8)) !limted to 4 sets of parameters
        allocate(tmpav_lag(30,msub))
    
        allocate (rch_hwattemp(msub,nstep_rt))
